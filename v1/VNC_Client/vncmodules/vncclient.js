...
class VncClient extends Events {
  ...
  constructor(
    options = { debug: false, fps: 0, encodings: [], debugLevel: 1 }
  ) {
    super();
  ...
  requestFrameUpdate(
    ...
    if (
      ...
      // Request data
      const message = new Buffer.alloc(10);
  ...
  sendData(data, flush = true) {
    ...
    if (this._connection) this._connection.write(data);
  }
  ...
  updateFbSize() {
    ...
    this.fb = new Buffer.alloc(this.clientWidth * this.clientHeight * 4);
  }
  ...
  _sendEncodings() {
    ...
    const message = new Buffer.alloc(
