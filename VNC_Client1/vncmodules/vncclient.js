...
const LOG_ERROR = 0;
// unused const LOG_WARN = 1;
const LOG_INFO = 2;
// unused const LOG_HTTP = 3;
const LOG_VERBOSE = 4;
const LOG_DEBUG = 5;
// unused const LOG_SILLY = 6;
...
class VncClient extends Events {
  ...
  constructor(
    options = { debug: false, fps: 0, encodings: [], debugLevel: 2 }
  ) {
    super();
    ...
    this.debugLevel = options.debugLevel || LOG_INFO;
  ...
  requestFrameUpdate(
    ...
    if (
      ...
      this._log("Requesting frame update.", LOG_VERBOSE);
  ...
  async _handleAuthChallenge() {
    if (this._challengeResponseSent) {
      ...
      this._log(`Sending response: ${response.toString()}`, LOG_VERBOSE);
  ...
  async _handleServerInit() {
    ...
    this.pixelFormat.redMax = this.bigEndianFlag
      ? this._socketBuffer.readUInt16BE()
      : this._socketBuffer.readUInt16LE();
    this.pixelFormat.greenMax = this.bigEndianFlag
      ? this._socketBuffer.readUInt16BE()
      : this._socketBuffer.readUInt16LE();
    this.pixelFormat.blueMax = this.bigEndianFlag
      ? this._socketBuffer.readUInt16BE()
      : this._socketBuffer.readUInt16LE();
    ...
    if (this._set8BitColor) {
      this._log(
        "8 bit color format requested, only raw encoding is supported.",
        LOG_VERBOSE
      );
   ...
   */
  updateFbSize() {
    this._log("Updating the frame buffer size.", LOG_VERBOSE);
  ...
  _setPixelFormatToColorMap() {
    this._log(
      "Requesting PixelFormat change to ColorMap (8 bits).",
      LOG_VERBOSE
    );
  ...
  _sendEncodings() {
    ...
    const message = Buffer.alloc(
      4 + (!this._set8BitColor ? this.encodings.length : 2) * 4
    );
  ...
  async _handleCutText() {
    ...
    this.emit(
      "cutText",
      this._socketBuffer.readNBytesOffset(length).toString()
    );
    this._socketBuffer.flush();
  }
  ...
  async _handleRect() {
    ...
    while (this._rects) {
      ...
      if (rect.encoding === encodings.pseudoQemuAudio) {
        ...
        const dataSize =
          rect.width * rect.height * (this.pixelFormat.bitsPerPixel / 8);
        ...
        this._cursor.cursorPixels =
          this._socketBuffer.readNBytesOffset(dataSize);
        this._cursor.bitmask = this._socketBuffer.readNBytesOffset(bitmaskSize);
        rect.data = Buffer.concat([
          this._cursor.cursorPixels,
          this._cursor.bitmask,
        ]);
      } else if (rect.encoding === encodings.pseudoDesktopSize) {
        this._log(
          "Frame Buffer size change requested by the server",
          LOG_VERBOSE
        );
    ...
    if (sendFbUpdate) {
      ...
      this._log("Frame buffer updated.", LOG_VERBOSE);
  ...
  async _handleFbUpdate() {
    ...
    this._log(`Frame update received. Rects: ${this._rects}`, LOG_VERBOSE);
  ...
  async _handleSetColorMap() {
    ...
    for (let x = 0; x < numColors; x++) {
      this._colorMap[firstColor] = this._socketBuffer.readRgbColorMap(
        this.pixelFormat.redShift,
        this.pixelFormat.greenShift,
        this.pixelFormat.blueShift,
        this.pixelFormat.redMax,
        this.pixelFormat.greenMax,
        this.pixelFormat.blueMax
      );
      firstColor++;
    }
  ...
  async _handleQemuAudio() {
    ...
    if (operation === 2) {
      ...
      for (let i = 0; i < length / 2; i++) {
        audioBuffer.push(this._socketBuffer.readUInt16BE());
      }
    ...
    if (!this._cursor.width) {
      ...
      for (let h = 0; h < this._cursor.height; h++) {
        for (let w = 0; w < this._cursor.width; w++) {
          const fbBytePosOffset = this._getPixelBytePos(
            this._cursor.posX + w,
            this._cursor.posY + h,
            this.clientWidth,
            this.clientHeight
          );
          const cursorBytePosOffset = this._getPixelBytePos(
            w,
            h,
            this._cursor.width,
            this._cursor.height
          );
          const bitmapByte = this._cursor.bitmask.slice(
            Math.floor(cursorBytePosOffset / 4 / 8),
          ...
          if (activePixel) {
            if (this.pixelFormat.bitsPerPixel === 8) {
              const index =
                this._cursor.cursorPixels.readUInt8(cursorBytePosOffset);
              ...
              const color = this._cursor.cursorPixels.readIntBE(
                cursorBytePosOffset,
                bytesLength
              );
