...
class Zrle {
  constructor(debug = false, debugLevel = 2) {
    this.debug = debug;
    ...
    this.zlib.on('error',err=>{
      this._log(`Error:${err}`,0);
    });
  }
  ...
  async decode(rect, fb, bitsPerPixel, colorMap, screenW, screenH, socket, depth, red, green, blue) {
    ...
    this.zlib.write(compressedData, async () => {
      ...
      this._log(`Starting rect processing. ${rect.width}x${rect.height}. Compressed size: ${
        dataSize}. Decompressed size: ${this.unBuffer.bytesLeft()}`, 3
      );
      ...
      while (tiles) {
        ...
        if (subEncoding === 129) {
          ...
          for (let x = 0; x < paletteSize; x++) {
            ...
            if (firstRle) {
              this._log(`Cor da paleta: ${  JSON.stringify(color)}`);
            }
        ...
        this._log(`Processing tile ${totalTiles - tiles}/${totalTiles} - SubEnc: ${
          subEncoding} - Size: ${tw}x${th} - BytesUsed: ${this.unBuffer.offset - initialOffset} - TotalRun: ${
          totalRun} - Runs: ${runs} - PaletteSize: ${palette.length}`, 3
        );
