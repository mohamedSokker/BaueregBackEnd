...
const getAllSite_Performance_piles = (req,res) => {
    var query = 'SELECT * FROM Site_Performance_piles';
    let { cond, limit, fullquery } = req.query;
    if (limit) {
        query = 'SELECT TOP ' + limit + ' * FROM Site_Performance_piles'
    } else {
        query = 'SELECT * FROM Site_Performance_piles'
    }
    if (cond) {
...
const getSite_Performance_piles = (req,res) => {
    var query = 'SELECT * FROM Site_Performance_piles';
    // console.log(query);
    sql.connect(config, function (err) {
        if (err) console.log(err);
...
const addSite_Performance_piles = (req,res) => {
    var getquery = "SELECT name FROM sys.columns WHERE object_id = OBJECT_ID('Site_Performance_piles')";
    var Results = [];
    // console.log(query);
    sql.connect(config).then(() => {
        ...
        request.query(getquery, function (err, recordsets) {
            ...
            Results = recordsets.recordsets[0]
            let keysStatus = true
            var query = "INSERT INTO Site_Performance_piles Values( "
...
const updateSite_Performance_piles = (req,res) => {
    var getquery = "SELECT name FROM sys.columns WHERE object_id = OBJECT_ID('Site_Performance_piles')";
    var Results = [];
    var cond = '';
    // console.log(query);
    sql.connect(config).then(() => {
        ...
        request.query(getquery, function (err, recordsets) {
            ...
            Results = recordsets.recordsets[0]
            let keysStatus = true
            var query = "UPDATE Site_Performance_piles SET "
            const keys = Object.keys(req.body)
            console.log(req.body)
            // var i = 0
            for (let i = 0; i < Results.length; i++) {
...
const deleteSite_Performance_piles = (req,res) => {
    // var query = 'SELECT * FROM Site_Performance_piles';
    // console.log(query);
    ...
    sql.connect(config, function (err) {
        ...
        var request = new sql.Request();
        //Read Sql Statment From File
        request.query("DELETE FROM Site_Performance_piles " +
