...
const getAllSite_Performance = (req,res) => {
    var query = 'SELECT * FROM Site_Performance';
    let { cond, limit, fullquery } = req.query;
    if (limit) {
        query = 'SELECT TOP ' + limit + ' * FROM Site_Performance'
    } else {
...
const getSite_Performance = (req,res) => {
    var query = 'SELECT * FROM Site_Performance';
    // console.log(query);
    sql.connect(config, function (err) {
        if (err) console.log(err);
...
const addSite_Performance = (req,res) => {
    var getquery = "SELECT name FROM sys.columns WHERE object_id = OBJECT_ID('Site_Performance')";
    var Results = [];
    // console.log(query);
    sql.connect(config).then(() => {
        ...
        request.query(getquery, function (err, recordsets) {
            ...
            Results = recordsets.recordsets[0]
            let keysStatus = true
            var query = "INSERT INTO Site_Performance Values( "
...
const updateSite_Performance = (req,res) => {
    var getquery = "SELECT name FROM sys.columns WHERE object_id = OBJECT_ID('Site_Performance')";
    var Results = [];
    var cond = '';
    // console.log(query);
    sql.connect(config).then(() => {
        ...
        request.query(getquery, function (err, recordsets) {
            ...
            Results = recordsets.recordsets[0]
            let keysStatus = true
            var query = "UPDATE Site_Performance SET "
            const keys = Object.keys(req.body)
            console.log(req.body)
            // var i = 0
            for (let i = 0; i < Results.length; i++) {
...
const deleteSite_Performance = (req,res) => {
    // var query = 'SELECT * FROM Site_Performance';
    // console.log(query);
    ...
    sql.connect(config, function (err) {
        ...
        var request = new sql.Request();
        //Read Sql Statment From File
        request.query("DELETE FROM Site_Performance " +
