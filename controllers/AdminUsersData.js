...
const getAllAdminUsersData = (req,res) => {
    var query = 'SELECT * FROM AdminUsersData';
    let { cond, limit, fullquery } = req.query;
    if (limit) {
        query = 'SELECT TOP ' + limit + ' * FROM AdminUsersData'
    } else {
        query = 'SELECT * FROM AdminUsersData'
    }
    if (cond) {
...
const getAdminUsersData = (req,res) => {
    var query = 'SELECT * FROM AdminUsersData';
    // console.log(query);
    sql.connect(config, function (err) {
        if (err) console.log(err);
...
const addAdminUsersData = (req,res) => {
    var getquery = "SELECT name FROM sys.columns WHERE object_id = OBJECT_ID('AdminUsersData')";
    var Results = [];
    // console.log(query);
    sql.connect(config).then(() => {
        ...
        request.query(getquery, function (err, recordsets) {
            ...
            Results = recordsets.recordsets[0]
            let keysStatus = true
            var query = "INSERT INTO AdminUsersData Values( "
...
const updateAdminUsersData = (req,res) => {
    var getquery = "SELECT name FROM sys.columns WHERE object_id = OBJECT_ID('AdminUsersData')";
    var Results = [];
    var cond = '';
    // console.log(query);
    sql.connect(config).then(() => {
        ...
        request.query(getquery, function (err, recordsets) {
            ...
            Results = recordsets.recordsets[0]
            let keysStatus = true
            var query = "UPDATE AdminUsersData SET "
            const keys = Object.keys(req.body)
            console.log(req.body)
            // var i = 0
            for (let i = 0; i < Results.length; i++) {
...
const deleteAdminUsersData = (req,res) => {
    // var query = 'SELECT * FROM AdminUsersData';
    // console.log(query);
    ...
    sql.connect(config, function (err) {
        ...
        var request = new sql.Request();
        //Read Sql Statment From File
        request.query("DELETE FROM AdminUsersData " +
