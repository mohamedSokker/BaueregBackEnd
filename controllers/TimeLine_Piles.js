...
const getAllTimeLine_Piles = (req,res) => {
    var query = 'SELECT * FROM TimeLine_Piles';
    let { cond, limit, fullquery } = req.query;
    if (limit) {
        query = 'SELECT TOP ' + limit + ' * FROM TimeLine_Piles'
    } else {
        query = 'SELECT * FROM TimeLine_Piles'
    }
    if (cond) {
...
const getTimeLine_Piles = (req,res) => {
    var query = 'SELECT * FROM TimeLine_Piles';
    // console.log(query);
    sql.connect(config, function (err) {
        if (err) console.log(err);
...
const addTimeLine_Piles = (req,res) => {
    var getquery = "SELECT name FROM sys.columns WHERE object_id = OBJECT_ID('TimeLine_Piles')";
    var Results = [];
    // console.log(query);
    sql.connect(config).then(() => {
        ...
        request.query(getquery, function (err, recordsets) {
            ...
            Results = recordsets.recordsets[0]
            let keysStatus = true
            var query = "INSERT INTO TimeLine_Piles Values( "
...
const updateTimeLine_Piles = (req,res) => {
    var getquery = "SELECT name FROM sys.columns WHERE object_id = OBJECT_ID('TimeLine_Piles')";
    var Results = [];
    var cond = '';
    // console.log(query);
    sql.connect(config).then(() => {
        ...
        request.query(getquery, function (err, recordsets) {
            ...
            Results = recordsets.recordsets[0]
            let keysStatus = true
            var query = "UPDATE TimeLine_Piles SET "
            const keys = Object.keys(req.body)
            console.log(req.body)
            // var i = 0
            for (let i = 0; i < Results.length; i++) {
...
const deleteTimeLine_Piles = (req,res) => {
    // var query = 'SELECT * FROM TimeLine_Piles';
    // console.log(query);
    ...
    sql.connect(config, function (err) {
        ...
        var request = new sql.Request();
        //Read Sql Statment From File
        request.query("DELETE FROM TimeLine_Piles " +
