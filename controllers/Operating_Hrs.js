...
const getAllOperating_Hrs = (req,res) => {
    var query = 'SELECT * FROM Operating_Hrs';
    let { cond, limit, fullquery } = req.query;
    if (limit) {
        query = 'SELECT TOP ' + limit + ' * FROM Operating_Hrs'
    } else {
        query = 'SELECT * FROM Operating_Hrs'
    }
    if (cond) {
...
const getOperating_Hrs = (req,res) => {
    var query = 'SELECT * FROM Operating_Hrs';
    // console.log(query);
    sql.connect(config, function (err) {
        if (err) console.log(err);
...
const addOperating_Hrs = (req,res) => {
    var getquery = "SELECT name FROM sys.columns WHERE object_id = OBJECT_ID('Operating_Hrs')";
    var Results = [];
    // console.log(query);
    sql.connect(config).then(() => {
        ...
        request.query(getquery, function (err, recordsets) {
            ...
            Results = recordsets.recordsets[0]
            let keysStatus = true
            var query = "INSERT INTO Operating_Hrs Values( "
...
const updateOperating_Hrs = (req,res) => {
    var getquery = "SELECT name FROM sys.columns WHERE object_id = OBJECT_ID('Operating_Hrs')";
    var Results = [];
    var cond = '';
    // console.log(query);
    sql.connect(config).then(() => {
        ...
        request.query(getquery, function (err, recordsets) {
            ...
            Results = recordsets.recordsets[0]
            let keysStatus = true
            var query = "UPDATE Operating_Hrs SET "
            const keys = Object.keys(req.body)
            console.log(req.body)
            // var i = 0
            for (let i = 0; i < Results.length; i++) {
...
const deleteOperating_Hrs = (req,res) => {
    // var query = 'SELECT * FROM Operating_Hrs';
    // console.log(query);
    ...
    sql.connect(config, function (err) {
        ...
        var request = new sql.Request();
        //Read Sql Statment From File
        request.query("DELETE FROM Operating_Hrs " +
