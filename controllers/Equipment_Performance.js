...
const getAllEquipment_Performance = (req,res) => {
    var query = 'SELECT * FROM Equipment_Performance';
    let { cond, limit, fullquery } = req.query;
    if (limit) {
        query = 'SELECT TOP ' + limit + ' * FROM Equipment_Performance'
    } else {
        query = 'SELECT * FROM Equipment_Performance'
    }
    if (cond) {
...
const getEquipment_Performance = (req,res) => {
    var query = 'SELECT * FROM Equipment_Performance';
    // console.log(query);
    sql.connect(config, function (err) {
        if (err) console.log(err);
...
const addEquipment_Performance = (req,res) => {
    var getquery = "SELECT name FROM sys.columns WHERE object_id = OBJECT_ID('Equipment_Performance')";
    var Results = [];
    // console.log(query);
    sql.connect(config).then(() => {
        ...
        request.query(getquery, function (err, recordsets) {
            ...
            Results = recordsets.recordsets[0]
            let keysStatus = true
            var query = "INSERT INTO Equipment_Performance Values( "
...
const updateEquipment_Performance = (req,res) => {
    var getquery = "SELECT name FROM sys.columns WHERE object_id = OBJECT_ID('Equipment_Performance')";
    var Results = [];
    var cond = '';
    // console.log(query);
    sql.connect(config).then(() => {
        ...
        request.query(getquery, function (err, recordsets) {
            ...
            Results = recordsets.recordsets[0]
            let keysStatus = true
            var query = "UPDATE Equipment_Performance SET "
            const keys = Object.keys(req.body)
            console.log(req.body)
            // var i = 0
            for (let i = 0; i < Results.length; i++) {
...
const deleteEquipment_Performance = (req,res) => {
    // var query = 'SELECT * FROM Equipment_Performance';
    // console.log(query);
    ...
    sql.connect(config, function (err) {
        ...
        var request = new sql.Request();
        //Read Sql Statment From File
        request.query("DELETE FROM Equipment_Performance " +
