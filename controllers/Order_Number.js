...
const getAllOrder_Number = (req,res) => {
    var query = 'SELECT * FROM Order_Number';
    let { cond, limit, fullquery } = req.query;
    if (limit) {
        query = 'SELECT TOP ' + limit + ' * FROM Order_Number'
    } else {
        query = 'SELECT * FROM Order_Number'
    }
    if (cond) {
...
const addOrder_Number = (req,res) => {
    var getquery = "SELECT name FROM sys.columns WHERE object_id = OBJECT_ID('Order_Number')";
    var Results = [];
    // console.log(query);
    sql.connect(config).then(() => {
        ...
        request.query(getquery, function (err, recordsets) {
            ...
            Results = recordsets.recordsets[0]
            let keysStatus = true
            var query = "INSERT INTO Order_Number Values( "
...
const updateOrder_Number = (req,res) => {
    var getquery = "SELECT name FROM sys.columns WHERE object_id = OBJECT_ID('Order_Number')";
    var Results = [];
    var cond = '';
    // console.log(query);
    sql.connect(config).then(() => {
        ...
        request.query(getquery, function (err, recordsets) {
            ...
            Results = recordsets.recordsets[0]
            let keysStatus = true
            var query = "UPDATE Order_Number SET "
            const keys = Object.keys(req.body)
            console.log(req.body)
            // var i = 0
            for (let i = 0; i < Results.length; i++) {
...
const deleteOrder_Number = (req,res) => {
    // var query = 'SELECT * FROM Order_Number';
    // console.log(query);
    ...
    sql.connect(config, function (err) {
        ...
        var request = new sql.Request();
        //Read Sql Statment From File
        request.query("DELETE FROM Order_Number " +
