...
const getAllStocks_Equipments = (req,res) => {
    var query = 'SELECT * FROM Stocks_Equipments';
    let { cond, limit, fullquery } = req.query;
    if (limit) {
        query = 'SELECT TOP ' + limit + ' * FROM Stocks_Equipments'
    } else {
        query = 'SELECT * FROM Stocks_Equipments'
    }
    if (cond) {
...
const getStocks_Equipments = (req,res) => {
    var query = 'SELECT * FROM Stocks_Equipments';
    // console.log(query);
    sql.connect(config, function (err) {
        if (err) console.log(err);
...
const addStocks_Equipments = (req,res) => {
    var getquery = "SELECT name FROM sys.columns WHERE object_id = OBJECT_ID('Stocks_Equipments')";
    var Results = [];
    // console.log(query);
    sql.connect(config).then(() => {
        ...
        request.query(getquery, function (err, recordsets) {
            ...
            Results = recordsets.recordsets[0]
            let keysStatus = true
            var query = "INSERT INTO Stocks_Equipments Values( "
...
const updateStocks_Equipments = (req,res) => {
    var getquery = "SELECT name FROM sys.columns WHERE object_id = OBJECT_ID('Stocks_Equipments')";
    var Results = [];
    var cond = '';
    // console.log(query);
    sql.connect(config).then(() => {
        ...
        request.query(getquery, function (err, recordsets) {
            ...
            Results = recordsets.recordsets[0]
            let keysStatus = true
            var query = "UPDATE Stocks_Equipments SET "
            const keys = Object.keys(req.body)
            console.log(req.body)
            // var i = 0
            for (let i = 0; i < Results.length; i++) {
...
const deleteStocks_Equipments = (req,res) => {
    // var query = 'SELECT * FROM Stocks_Equipments';
    // console.log(query);
    ...
    sql.connect(config, function (err) {
        ...
        var request = new sql.Request();
        //Read Sql Statment From File
        request.query("DELETE FROM Stocks_Equipments " +
...
module.exports = {
    getAllStocks_Equipments,
    getStocks_Equipments,
    addStocks_Equipments,
    updateStocks_Equipments,
    deleteStocks_Equipments
}