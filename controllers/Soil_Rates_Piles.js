...
const getAllSoil_Rates_Piles = (req,res) => {
    var query = 'SELECT * FROM Soil_Rates_Piles';
    let { cond, limit, fullquery } = req.query;
    if (limit) {
        query = 'SELECT TOP ' + limit + ' * FROM Soil_Rates_Piles'
    } else {
        query = 'SELECT * FROM Soil_Rates_Piles'
    }
    if (cond) {
...
const getSoil_Rates_Piles = (req,res) => {
    var query = 'SELECT * FROM Soil_Rates_Piles';
    // console.log(query);
    sql.connect(config, function (err) {
        if (err) console.log(err);
...
const addSoil_Rates_Piles = (req,res) => {
    var getquery = "SELECT name FROM sys.columns WHERE object_id = OBJECT_ID('Soil_Rates_Piles')";
    var Results = [];
    // console.log(query);
    sql.connect(config).then(() => {
        ...
        request.query(getquery, function (err, recordsets) {
            ...
            Results = recordsets.recordsets[0]
            let keysStatus = true
            var query = "INSERT INTO Soil_Rates_Piles Values( "
...
const updateSoil_Rates_Piles = (req,res) => {
    var getquery = "SELECT name FROM sys.columns WHERE object_id = OBJECT_ID('Soil_Rates_Piles')";
    var Results = [];
    var cond = '';
    // console.log(query);
    sql.connect(config).then(() => {
        ...
        request.query(getquery, function (err, recordsets) {
            ...
            Results = recordsets.recordsets[0]
            let keysStatus = true
            var query = "UPDATE Soil_Rates_Piles SET "
            const keys = Object.keys(req.body)
            console.log(req.body)
            // var i = 0
            for (let i = 0; i < Results.length; i++) {
...
const deleteSoil_Rates_Piles = (req,res) => {
    // var query = 'SELECT * FROM Soil_Rates_Piles';
    // console.log(query);
    ...
    sql.connect(config, function (err) {
        ...
        var request = new sql.Request();
        //Read Sql Statment From File
        request.query("DELETE FROM Soil_Rates_Piles " +
...
module.exports = {
    getAllSoil_Rates_Piles,
    getSoil_Rates_Piles,
    addSoil_Rates_Piles,
    updateSoil_Rates_Piles,
    deleteSoil_Rates_Piles
}