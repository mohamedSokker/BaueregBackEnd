...
const getAllStocks = (req,res) => {
    ...
    if (limit) {
        query = 'SELECT TOP ' + limit + ' * FROM Stocks'
    } else {
...
const addStocks = (req,res) => {
    var getquery = "SELECT name FROM sys.columns WHERE object_id = OBJECT_ID('Stocks')";
    var Results = [];
    // console.log(query);
    sql.connect(config).then(() => {
        ...
        request.query(getquery, function (err, recordsets) {
            ...
            Results = recordsets.recordsets[0]
            let keysStatus = true
            var query = "INSERT INTO Stocks Values( "
...
const deleteStocks = (req,res) => {
    ...
    sql.connect(config, function (err) {
        ...
        var request = new sql.Request();
        //Read Sql Statment From File
        request.query("DELETE FROM Stocks " +
