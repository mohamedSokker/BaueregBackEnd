...
const getAllOrder_Quotation = (req,res) => {
    var query = 'SELECT * FROM Order_Quotation';
    let { cond, limit, fullquery } = req.query;
    if (limit) {
        query = 'SELECT TOP ' + limit + ' * FROM Order_Quotation'
    } else {
...
const getOrder_Quotation = (req,res) => {
    var query = 'SELECT * FROM Order_Quotation';
    // console.log(query);
    sql.connect(config, function (err) {
        if (err) console.log(err);
...
const addOrder_Quotation = (req,res) => {
    var getquery = "SELECT name FROM sys.columns WHERE object_id = OBJECT_ID('Order_Quotation')";
    var Results = [];
    // console.log(query);
    sql.connect(config).then(() => {
        ...
        request.query(getquery, function (err, recordsets) {
            ...
            Results = recordsets.recordsets[0]
            let keysStatus = true
            var query = "INSERT INTO Order_Quotation Values( "
...
const updateOrder_Quotation = (req,res) => {
    var getquery = "SELECT name FROM sys.columns WHERE object_id = OBJECT_ID('Order_Quotation')";
    var Results = [];
    var cond = '';
    // console.log(query);
    sql.connect(config).then(() => {
        ...
        request.query(getquery, function (err, recordsets) {
            ...
            Results = recordsets.recordsets[0]
            let keysStatus = true
            var query = "UPDATE Order_Quotation SET "
            const keys = Object.keys(req.body)
            console.log(req.body)
            // var i = 0
            for (let i = 0; i < Results.length; i++) {
...
const deleteOrder_Quotation = (req,res) => {
    // var query = 'SELECT * FROM Order_Quotation';
    // console.log(query);
    ...
    sql.connect(config, function (err) {
        ...
        var request = new sql.Request();
        //Read Sql Statment From File
        request.query("DELETE FROM Order_Quotation " +
