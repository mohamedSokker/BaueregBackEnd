...
const getAllBauer_Equipments = (req, res) => {
  var query = "SELECT * FROM Bauer_Equipments";
  let { cond, limit, fullquery } = req.query;
  if (limit) {
    query = "SELECT TOP " + limit + " * FROM Bauer_Equipments";
  } else {
    query = "SELECT * FROM Bauer_Equipments";
  }
  if (cond) {
...
const getBauer_Equipments = (req, res) => {
  var query = "SELECT * FROM Bauer_Equipments";
  // console.log(query);
  sql.connect(config, function (err) {
    if (err) console.log(err);
...
const addBauer_Equipments = (req, res) => {
  var getquery =
    "SELECT name FROM sys.columns WHERE object_id = OBJECT_ID('Bauer_Equipments')";
  var Results = [];
  // console.log(query);
  sql.connect(config).then(() => {
    ...
    request.query(getquery, function (err, recordsets) {
      ...
      Results = recordsets.recordsets[0];
      let keysStatus = true;
      var query = "INSERT INTO Bauer_Equipments Values( ";
...
const updateBauer_Equipments = (req, res) => {
  var getquery =
    "SELECT name FROM sys.columns WHERE object_id = OBJECT_ID('Bauer_Equipments')";
  var Results = [];
  var cond = "";
  ...
  sql.connect(config).then(() => {
    ...
    request.query(getquery, function (err, recordsets) {
      ...
      Results = recordsets.recordsets[0];
      let keysStatus = true;
      var query = "UPDATE Bauer_Equipments SET ";
      const keys = Object.keys(req.body);
      console.log(req.body);
      // var i = 0
      for (let i = 0; i < Results.length; i++) {
...
const deleteBauer_Equipments = (req, res) => {
  // var query = 'SELECT * FROM Bauer_Equipments';
  // console.log(query);
