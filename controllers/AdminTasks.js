...
const getAllAdminTasks = (req, res) => {
  var query = "SELECT * FROM AdminTasks ORDER BY ID DESC";
  let { cond, limit, fullquery } = req.query;
  if (limit) {
    query = "SELECT TOP " + limit + " * FROM AdminTasks";
  } else {
    query = "SELECT * FROM AdminTasks ORDER BY ID DESC";
  }
  if (cond) {
...
const getAdminTasks = (req, res) => {
  var query = "SELECT * FROM AdminTasks";
  // console.log(query);
  sql.connect(config, function (err) {
    ...
    request.query(query, function (err, recordsets) {
      ...
      const SearchedItems = Results.find(
        (Result) => Result.UserID == Object.values(req.params)[0]
      );
...
const addAdminTasks = (req, res) => {
  var getquery =
    "SELECT name FROM sys.columns WHERE object_id = OBJECT_ID('AdminTasks')";
  var Results = [];
  // console.log(query);
  sql.connect(config).then(() => {
    ...
    request.query(getquery, function (err, recordsets) {
      ...
      Results = recordsets.recordsets[0];
      let keysStatus = true;
      var query = "INSERT INTO AdminTasks Values( ";
...
const updateAdminTasks = (req, res) => {
  var getquery =
    "SELECT name FROM sys.columns WHERE object_id = OBJECT_ID('AdminTasks')";
  var Results = [];
  var cond = "";
  ...
  sql.connect(config).then(() => {
    ...
    request.query(getquery, function (err, recordsets) {
      ...
      Results = recordsets.recordsets[0];
      let keysStatus = true;
      var query = "UPDATE AdminTasks SET ";
      const keys = Object.keys(req.body);
      console.log(req.body);
      // var i = 0
      for (let i = 0; i < Results.length; i++) {
...
const deleteAdminTasks = (req, res) => {
  // var query = 'SELECT * FROM AdminTasks';
  // console.log(query);
