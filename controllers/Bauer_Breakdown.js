...
const getAllBauer_Breakdown = (req,res) => {
    var query = 'SELECT * FROM Bauer_Breakdown';
    let { cond, limit, fullquery } = req.query;
    if (limit) {
        query = 'SELECT TOP ' + limit + ' * FROM Bauer_Breakdown'
    } else {
        query = 'SELECT * FROM Bauer_Breakdown'
    }
    if (cond) {
...
const getBauer_Breakdown = (req,res) => {
    var query = 'SELECT * FROM Bauer_Breakdown';
    // console.log(query);
    sql.connect(config, function (err) {
        if (err) console.log(err);
...
const addBauer_Breakdown = (req,res) => {
    var getquery = "SELECT name FROM sys.columns WHERE object_id = OBJECT_ID('Bauer_Breakdown')";
    var Results = [];
    // console.log(query);
    sql.connect(config).then(() => {
        ...
        request.query(getquery, function (err, recordsets) {
            ...
            Results = recordsets.recordsets[0]
            let keysStatus = true
            var query = "INSERT INTO Bauer_Breakdown Values( "
...
const updateBauer_Breakdown = (req,res) => {
    var getquery = "SELECT name FROM sys.columns WHERE object_id = OBJECT_ID('Bauer_Breakdown')";
    var Results = [];
    var cond = '';
    // console.log(query);
    sql.connect(config).then(() => {
        ...
        request.query(getquery, function (err, recordsets) {
            ...
            Results = recordsets.recordsets[0]
            let keysStatus = true
            var query = "UPDATE Bauer_Breakdown SET "
            const keys = Object.keys(req.body)
            console.log(req.body)
            // var i = 0
            for (let i = 0; i < Results.length; i++) {
...
const deleteBauer_Breakdown = (req,res) => {
    // var query = 'SELECT * FROM Bauer_Breakdown';
    // console.log(query);
    ...
    sql.connect(config, function (err) {
        ...
        var request = new sql.Request();
        //Read Sql Statment From File
        request.query("DELETE FROM Bauer_Breakdown " +
