...
const getAllKelly_Location = (req,res) => {
    var query = 'SELECT * FROM Kelly_Location';
    let { cond, limit, fullquery } = req.query;
    if (limit) {
        query = 'SELECT TOP ' + limit + ' * FROM Kelly_Location'
    } else {
        query = 'SELECT * FROM Kelly_Location'
    }
    if (cond) {
...
const getKelly_Location = (req,res) => {
    var query = 'SELECT * FROM Kelly_Location';
    // console.log(query);
    sql.connect(config, function (err) {
        if (err) console.log(err);
...
const addKelly_Location = (req,res) => {
    var getquery = "SELECT name FROM sys.columns WHERE object_id = OBJECT_ID('Kelly_Location')";
    var Results = [];
    // console.log(query);
    sql.connect(config).then(() => {
        ...
        request.query(getquery, function (err, recordsets) {
            ...
            Results = recordsets.recordsets[0]
            let keysStatus = true
            var query = "INSERT INTO Kelly_Location Values( "
...
const updateKelly_Location = (req,res) => {
    var getquery = "SELECT name FROM sys.columns WHERE object_id = OBJECT_ID('Kelly_Location')";
    var Results = [];
    var cond = '';
    // console.log(query);
    sql.connect(config).then(() => {
        ...
        request.query(getquery, function (err, recordsets) {
            ...
            Results = recordsets.recordsets[0]
            let keysStatus = true
            var query = "UPDATE Kelly_Location SET "
            const keys = Object.keys(req.body)
            console.log(req.body)
            // var i = 0
            for (let i = 0; i < Results.length; i++) {
...
const deleteKelly_Location = (req,res) => {
    // var query = 'SELECT * FROM Kelly_Location';
    // console.log(query);
    ...
    sql.connect(config, function (err) {
        ...
        var request = new sql.Request();
        //Read Sql Statment From File
        request.query("DELETE FROM Kelly_Location " +
