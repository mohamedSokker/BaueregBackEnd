...
const getAllOrder_EqTypeKeyWords = (req,res) => {
    var query = 'SELECT * FROM Order_EqTypeKeyWords';
    let { cond, limit, fullquery } = req.query;
    if (limit) {
        query = 'SELECT TOP ' + limit + ' * FROM Order_EqTypeKeyWords'
    } else {
        query = 'SELECT * FROM Order_EqTypeKeyWords'
    }
    if (cond) {
...
const getOrder_EqTypeKeyWords = (req,res) => {
    var query = 'SELECT * FROM Order_EqTypeKeyWords';
    // console.log(query);
    sql.connect(config, function (err) {
        if (err) console.log(err);
...
const addOrder_EqTypeKeyWords = (req,res) => {
    var getquery = "SELECT name FROM sys.columns WHERE object_id = OBJECT_ID('Order_EqTypeKeyWords')";
    var Results = [];
    // console.log(query);
    sql.connect(config).then(() => {
        ...
        request.query(getquery, function (err, recordsets) {
            ...
            Results = recordsets.recordsets[0]
            let keysStatus = true
            var query = "INSERT INTO Order_EqTypeKeyWords Values( "
...
const updateOrder_EqTypeKeyWords = (req,res) => {
    var getquery = "SELECT name FROM sys.columns WHERE object_id = OBJECT_ID('Order_EqTypeKeyWords')";
    var Results = [];
    var cond = '';
    // console.log(query);
    sql.connect(config).then(() => {
        ...
        request.query(getquery, function (err, recordsets) {
            ...
            Results = recordsets.recordsets[0]
            let keysStatus = true
            var query = "UPDATE Order_EqTypeKeyWords SET "
            const keys = Object.keys(req.body)
            console.log(req.body)
            // var i = 0
            for (let i = 0; i < Results.length; i++) {
...
const deleteOrder_EqTypeKeyWords = (req,res) => {
    // var query = 'SELECT * FROM Order_EqTypeKeyWords';
    // console.log(query);
    ...
    sql.connect(config, function (err) {
        ...
        var request = new sql.Request();
        //Read Sql Statment From File
        request.query("DELETE FROM Order_EqTypeKeyWords " +
