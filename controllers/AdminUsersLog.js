...
const getAllAdminUsersLog = (req,res) => {
    var query = 'SELECT * FROM AdminUsersLog';
    let { cond, limit, fullquery } = req.query;
    if (limit) {
        query = 'SELECT TOP ' + limit + ' * FROM AdminUsersLog'
    } else {
        query = 'SELECT * FROM AdminUsersLog'
    }
    if (cond) {
...
const getAdminUsersLog = (req,res) => {
    var query = 'SELECT * FROM AdminUsersLog';
    // console.log(query);
    sql.connect(config, function (err) {
        if (err) console.log(err);
...
const addAdminUsersLog = (req,res) => {
    var getquery = "SELECT name FROM sys.columns WHERE object_id = OBJECT_ID('AdminUsersLog')";
    var Results = [];
    // console.log(query);
    sql.connect(config).then(() => {
        ...
        request.query(getquery, function (err, recordsets) {
            ...
            Results = recordsets.recordsets[0]
            let keysStatus = true
            var query = "INSERT INTO AdminUsersLog Values( "
...
const updateAdminUsersLog = (req,res) => {
    var getquery = "SELECT name FROM sys.columns WHERE object_id = OBJECT_ID('AdminUsersLog')";
    var Results = [];
    var cond = '';
    // console.log(query);
    sql.connect(config).then(() => {
        ...
        request.query(getquery, function (err, recordsets) {
            ...
            Results = recordsets.recordsets[0]
            let keysStatus = true
            var query = "UPDATE AdminUsersLog SET "
            const keys = Object.keys(req.body)
            console.log(req.body)
            // var i = 0
            for (let i = 0; i < Results.length; i++) {
...
const deleteAdminUsersLog = (req,res) => {
    // var query = 'SELECT * FROM AdminUsersLog';
    // console.log(query);
    ...
    sql.connect(config, function (err) {
        ...
        var request = new sql.Request();
        //Read Sql Statment From File
        request.query("DELETE FROM AdminUsersLog " +
