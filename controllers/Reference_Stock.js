...
const getAllReference_Stock = (req,res) => {
    var query = 'SELECT * FROM Reference_Stock';
    let { cond, limit, fullquery } = req.query;
    if (limit) {
        query = 'SELECT TOP ' + limit + ' * FROM Reference_Stock'
    } else {
        query = 'SELECT * FROM Reference_Stock'
    }
    if (cond) {
...
const getReference_Stock = (req,res) => {
    var query = 'SELECT * FROM Reference_Stock';
    // console.log(query);
    sql.connect(config, function (err) {
        if (err) console.log(err);
...
const addReference_Stock = (req,res) => {
    var getquery = "SELECT name FROM sys.columns WHERE object_id = OBJECT_ID('Reference_Stock')";
    var Results = [];
    // console.log(query);
    sql.connect(config).then(() => {
        ...
        request.query(getquery, function (err, recordsets) {
            ...
            Results = recordsets.recordsets[0]
            let keysStatus = true
            var query = "INSERT INTO Reference_Stock Values( "
...
const updateReference_Stock = (req,res) => {
    var getquery = "SELECT name FROM sys.columns WHERE object_id = OBJECT_ID('Reference_Stock')";
    var Results = [];
    var cond = '';
    // console.log(query);
    sql.connect(config).then(() => {
        ...
        request.query(getquery, function (err, recordsets) {
            ...
            Results = recordsets.recordsets[0]
            let keysStatus = true
            var query = "UPDATE Reference_Stock SET "
            const keys = Object.keys(req.body)
            console.log(req.body)
            // var i = 0
            for (let i = 0; i < Results.length; i++) {
...
const deleteReference_Stock = (req,res) => {
    // var query = 'SELECT * FROM Reference_Stock';
    // console.log(query);
    ...
    sql.connect(config, function (err) {
        ...
        var request = new sql.Request();
        //Read Sql Statment From File
        request.query("DELETE FROM Reference_Stock " +
