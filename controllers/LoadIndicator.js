...
const getAllLoadIndicator = (req,res) => {
    var query = 'SELECT * FROM LoadIndicator';
    let { cond, limit, fullquery } = req.query;
    if (limit) {
        query = 'SELECT TOP ' + limit + ' * FROM LoadIndicator'
    } else {
        query = 'SELECT * FROM LoadIndicator'
    }
    if (cond) {
...
const getLoadIndicator = (req,res) => {
    var query = 'SELECT * FROM LoadIndicator';
    // console.log(query);
    sql.connect(config, function (err) {
        if (err) console.log(err);
...
const addLoadIndicator = (req,res) => {
    var getquery = "SELECT name FROM sys.columns WHERE object_id = OBJECT_ID('LoadIndicator')";
    var Results = [];
    // console.log(query);
    sql.connect(config).then(() => {
        ...
        request.query(getquery, function (err, recordsets) {
            ...
            Results = recordsets.recordsets[0]
            let keysStatus = true
            var query = "INSERT INTO LoadIndicator Values( "
...
const updateLoadIndicator = (req,res) => {
    var getquery = "SELECT name FROM sys.columns WHERE object_id = OBJECT_ID('LoadIndicator')";
    var Results = [];
    var cond = '';
    // console.log(query);
    sql.connect(config).then(() => {
        ...
        request.query(getquery, function (err, recordsets) {
            ...
            Results = recordsets.recordsets[0]
            let keysStatus = true
            var query = "UPDATE LoadIndicator SET "
            const keys = Object.keys(req.body)
            console.log(req.body)
            // var i = 0
            for (let i = 0; i < Results.length; i++) {
...
const deleteLoadIndicator = (req,res) => {
    // var query = 'SELECT * FROM LoadIndicator';
    // console.log(query);
    ...
    sql.connect(config, function (err) {
        ...
        var request = new sql.Request();
        //Read Sql Statment From File
        request.query("DELETE FROM LoadIndicator " +
