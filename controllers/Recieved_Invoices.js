...
const getAllRecieved_Invoices = (req,res) => {
    var query = 'SELECT * FROM Recieved_Invoices';
    let { cond, limit, fullquery } = req.query;
    if (limit) {
        query = 'SELECT TOP ' + limit + ' * FROM Recieved_Invoices'
    } else {
        query = 'SELECT * FROM Recieved_Invoices'
    }
    if (cond) {
...
const getRecieved_Invoices = (req,res) => {
    var query = 'SELECT * FROM Recieved_Invoices';
    // console.log(query);
    sql.connect(config, function (err) {
        if (err) console.log(err);
...
const addRecieved_Invoices = (req,res) => {
    var getquery = "SELECT name FROM sys.columns WHERE object_id = OBJECT_ID('Recieved_Invoices')";
    var Results = [];
    // console.log(query);
    sql.connect(config).then(() => {
        ...
        request.query(getquery, function (err, recordsets) {
            ...
            Results = recordsets.recordsets[0]
            let keysStatus = true
            var query = "INSERT INTO Recieved_Invoices Values( "
...
const updateRecieved_Invoices = (req,res) => {
    var getquery = "SELECT name FROM sys.columns WHERE object_id = OBJECT_ID('Recieved_Invoices')";
    var Results = [];
    var cond = '';
    // console.log(query);
    sql.connect(config).then(() => {
        ...
        request.query(getquery, function (err, recordsets) {
            ...
            Results = recordsets.recordsets[0]
            let keysStatus = true
            var query = "UPDATE Recieved_Invoices SET "
            const keys = Object.keys(req.body)
            console.log(req.body)
            // var i = 0
            for (let i = 0; i < Results.length; i++) {
...
const deleteRecieved_Invoices = (req,res) => {
    // var query = 'SELECT * FROM Recieved_Invoices';
    // console.log(query);
    ...
    sql.connect(config, function (err) {
        ...
        var request = new sql.Request();
        //Read Sql Statment From File
        request.query("DELETE FROM Recieved_Invoices " +
