...
const getAllAppPlaceOrder = (req, res) => {
  var query = "SELECT * FROM AppPlaceOrder";
  let { cond, limit, fullquery } = req.query;
  if (limit) {
    query = "SELECT TOP " + limit + " * FROM AppPlaceOrder";
  } else {
    query = "SELECT * FROM AppPlaceOrder";
  }
  if (cond) {
...
const getAppPlaceOrder = (req, res) => {
  var query = "SELECT * FROM AppPlaceOrder";
  // console.log(query);
  sql.connect(config, function (err) {
    if (err) console.log(err);
...
const addAppPlaceOrder = (req, res) => {
  var getquery =
    "SELECT name FROM sys.columns WHERE object_id = OBJECT_ID('AppPlaceOrder')";
  var Results = [];
  // console.log(query);
  sql.connect(config).then(() => {
    ...
    request.query(getquery, function (err, recordsets) {
      ...
      Results = recordsets.recordsets[0];
      let keysStatus = true;
      var query = "INSERT INTO AppPlaceOrder Values( ";
...
const updateAppPlaceOrder = (req, res) => {
  var getquery =
    "SELECT name FROM sys.columns WHERE object_id = OBJECT_ID('AppPlaceOrder')";
  var Results = [];
  var cond = "";
  ...
  sql.connect(config).then(() => {
    ...
    request.query(getquery, function (err, recordsets) {
      ...
      Results = recordsets.recordsets[0];
      let keysStatus = true;
      var query = "UPDATE AppPlaceOrder SET ";
      const keys = Object.keys(req.body);
      console.log(req.body);
      // var i = 0
      for (let i = 0; i < Results.length; i++) {
...
const deleteAppPlaceOrder = (req, res) => {
  // var query = 'SELECT * FROM AppPlaceOrder';
  // console.log(query);
