...
const getAllEquipment_Performance_Piles = (req,res) => {
    var query = 'SELECT * FROM Equipment_Performance_Piles';
    let { cond, limit, fullquery } = req.query;
    if (limit) {
        query = 'SELECT TOP ' + limit + ' * FROM Equipment_Performance_Piles'
    } else {
        query = 'SELECT * FROM Equipment_Performance_Piles'
    }
    if (cond) {
...
const getEquipment_Performance_Piles = (req,res) => {
    var query = 'SELECT * FROM Equipment_Performance_Piles';
    // console.log(query);
    sql.connect(config, function (err) {
        if (err) console.log(err);
...
const addEquipment_Performance_Piles = (req,res) => {
    var getquery = "SELECT name FROM sys.columns WHERE object_id = OBJECT_ID('Equipment_Performance_Piles')";
    var Results = [];
    // console.log(query);
    sql.connect(config).then(() => {
        ...
        request.query(getquery, function (err, recordsets) {
            ...
            Results = recordsets.recordsets[0]
            let keysStatus = true
            var query = "INSERT INTO Equipment_Performance_Piles Values( "
...
const updateEquipment_Performance_Piles = (req,res) => {
    var getquery = "SELECT name FROM sys.columns WHERE object_id = OBJECT_ID('Equipment_Performance_Piles')";
    var Results = [];
    var cond = '';
    // console.log(query);
    sql.connect(config).then(() => {
        ...
        request.query(getquery, function (err, recordsets) {
            ...
            Results = recordsets.recordsets[0]
            let keysStatus = true
            var query = "UPDATE Equipment_Performance_Piles SET "
            const keys = Object.keys(req.body)
            console.log(req.body)
            // var i = 0
            for (let i = 0; i < Results.length; i++) {
...
const deleteEquipment_Performance_Piles = (req,res) => {
    // var query = 'SELECT * FROM Equipment_Performance_Piles';
    // console.log(query);
    ...
    sql.connect(config, function (err) {
        ...
        var request = new sql.Request();
        //Read Sql Statment From File
        request.query("DELETE FROM Equipment_Performance_Piles " +
