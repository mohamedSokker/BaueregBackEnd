...
const getAllPeriodicMaintenance_Plan = (req,res) => {
    var query = 'SELECT * FROM PeriodicMaintenance_Plan';
    let { cond, limit, fullquery } = req.query;
    if (limit) {
        query = 'SELECT TOP ' + limit + ' * FROM PeriodicMaintenance_Plan'
    } else {
        query = 'SELECT * FROM PeriodicMaintenance_Plan'
    }
    if (cond) {
...
const getPeriodicMaintenance_Plan = (req,res) => {
    var query = 'SELECT * FROM PeriodicMaintenance_Plan';
    // console.log(query);
    sql.connect(config, function (err) {
        if (err) console.log(err);
...
const addPeriodicMaintenance_Plan = (req,res) => {
    var getquery = "SELECT name FROM sys.columns WHERE object_id = OBJECT_ID('PeriodicMaintenance_Plan')";
    var Results = [];
    // console.log(query);
    sql.connect(config).then(() => {
        ...
        request.query(getquery, function (err, recordsets) {
            ...
            Results = recordsets.recordsets[0]
            let keysStatus = true
            var query = "INSERT INTO PeriodicMaintenance_Plan Values( "
...
const updatePeriodicMaintenance_Plan = (req,res) => {
    var getquery = "SELECT name FROM sys.columns WHERE object_id = OBJECT_ID('PeriodicMaintenance_Plan')";
    var Results = [];
    var cond = '';
    // console.log(query);
    sql.connect(config).then(() => {
        ...
        request.query(getquery, function (err, recordsets) {
            ...
            Results = recordsets.recordsets[0]
            let keysStatus = true
            var query = "UPDATE PeriodicMaintenance_Plan SET "
            const keys = Object.keys(req.body)
            console.log(req.body)
            // var i = 0
            for (let i = 0; i < Results.length; i++) {
...
const deletePeriodicMaintenance_Plan = (req,res) => {
    // var query = 'SELECT * FROM PeriodicMaintenance_Plan';
    // console.log(query);
    ...
    sql.connect(config, function (err) {
        ...
        var request = new sql.Request();
        //Read Sql Statment From File
        request.query("DELETE FROM PeriodicMaintenance_Plan " +
