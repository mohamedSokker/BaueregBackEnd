...
const getAllGearBoxesTrench = (req,res) => {
    var query = 'SELECT * FROM GearBoxesTrench';
    let { cond, limit, fullquery } = req.query;
    if (limit) {
        query = 'SELECT TOP ' + limit + ' * FROM GearBoxesTrench'
    } else {
        query = 'SELECT * FROM GearBoxesTrench'
    }
    if (cond) {
...
const getGearBoxesTrench = (req,res) => {
    var query = 'SELECT * FROM GearBoxesTrench';
    // console.log(query);
    sql.connect(config, function (err) {
        if (err) console.log(err);
...
const addGearBoxesTrench = (req,res) => {
    var getquery = "SELECT name FROM sys.columns WHERE object_id = OBJECT_ID('GearBoxesTrench')";
    var Results = [];
    // console.log(query);
    sql.connect(config).then(() => {
        ...
        request.query(getquery, function (err, recordsets) {
            ...
            Results = recordsets.recordsets[0]
            let keysStatus = true
            var query = "INSERT INTO GearBoxesTrench Values( "
...
const updateGearBoxesTrench = (req,res) => {
    var getquery = "SELECT name FROM sys.columns WHERE object_id = OBJECT_ID('GearBoxesTrench')";
    var Results = [];
    var cond = '';
    // console.log(query);
    sql.connect(config).then(() => {
        ...
        request.query(getquery, function (err, recordsets) {
            ...
            Results = recordsets.recordsets[0]
            let keysStatus = true
            var query = "UPDATE GearBoxesTrench SET "
            const keys = Object.keys(req.body)
            console.log(req.body)
            // var i = 0
            for (let i = 0; i < Results.length; i++) {
...
const deleteGearBoxesTrench = (req,res) => {
    // var query = 'SELECT * FROM GearBoxesTrench';
    // console.log(query);
    ...
    sql.connect(config, function (err) {
        ...
        var request = new sql.Request();
        //Read Sql Statment From File
        request.query("DELETE FROM GearBoxesTrench " +
