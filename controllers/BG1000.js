...
const getAllBG1000 = (req,res) => {
    var query = 'SELECT * FROM BG1000';
    let { cond, limit, fullquery } = req.query;
    if (limit) {
        query = 'SELECT TOP ' + limit + ' * FROM BG1000'
    } else {
        query = 'SELECT * FROM BG1000'
    }
    if (cond) {
...
const addBG1000 = (req,res) => {
    var getquery = "SELECT name FROM sys.columns WHERE object_id = OBJECT_ID('BG1000')";
    var Results = [];
    // console.log(query);
    sql.connect(config).then(() => {
        ...
        request.query(getquery, function (err, recordsets) {
            ...
            Results = recordsets.recordsets[0]
            let keysStatus = true
            var query = "INSERT INTO BG1000 Values( "
...
const deleteBG1000 = (req,res) => {
    ...
    sql.connect(config, function (err) {
        ...
        var request = new sql.Request();
        //Read Sql Statment From File
        request.query("DELETE FROM BG1000 " +
