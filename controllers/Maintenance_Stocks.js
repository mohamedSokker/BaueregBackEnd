...
const getAllMaintenance_Stocks = (req,res) => {
    var query = 'SELECT * FROM Maintenance_Stocks';
    let { cond, limit, fullquery } = req.query;
    if (limit) {
        query = 'SELECT TOP ' + limit + ' * FROM Maintenance_Stocks'
    } else {
        query = 'SELECT * FROM Maintenance_Stocks'
    }
    if (cond) {
...
const getMaintenance_Stocks = (req,res) => {
    var query = 'SELECT * FROM Maintenance_Stocks';
    // console.log(query);
    sql.connect(config, function (err) {
        if (err) console.log(err);
...
const addMaintenance_Stocks = (req,res) => {
    var getquery = "SELECT name FROM sys.columns WHERE object_id = OBJECT_ID('Maintenance_Stocks')";
    var Results = [];
    // console.log(query);
    sql.connect(config).then(() => {
        ...
        request.query(getquery, function (err, recordsets) {
            ...
            Results = recordsets.recordsets[0]
            let keysStatus = true
            var query = "INSERT INTO Maintenance_Stocks Values( "
...
const updateMaintenance_Stocks = (req,res) => {
    var getquery = "SELECT name FROM sys.columns WHERE object_id = OBJECT_ID('Maintenance_Stocks')";
    var Results = [];
    var cond = '';
    // console.log(query);
    sql.connect(config).then(() => {
        ...
        request.query(getquery, function (err, recordsets) {
            ...
            Results = recordsets.recordsets[0]
            let keysStatus = true
            var query = "UPDATE Maintenance_Stocks SET "
            const keys = Object.keys(req.body)
            console.log(req.body)
            // var i = 0
            for (let i = 0; i < Results.length; i++) {
...
const deleteMaintenance_Stocks = (req,res) => {
    // var query = 'SELECT * FROM Maintenance_Stocks';
    // console.log(query);
    ...
    sql.connect(config, function (err) {
        ...
        var request = new sql.Request();
        //Read Sql Statment From File
        request.query("DELETE FROM Maintenance_Stocks " +
