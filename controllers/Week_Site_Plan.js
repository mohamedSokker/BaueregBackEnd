...
const getAllWeek_Site_Plan = (req,res) => {
    var query = 'SELECT * FROM Week_Site_Plan';
    let { cond, limit, fullquery } = req.query;
    if (limit) {
        query = 'SELECT TOP ' + limit + ' * FROM Week_Site_Plan'
    } else {
        query = 'SELECT * FROM Week_Site_Plan'
    }
    if (cond) {
...
const getWeek_Site_Plan = (req,res) => {
    var query = 'SELECT * FROM Week_Site_Plan';
    // console.log(query);
    sql.connect(config, function (err) {
        if (err) console.log(err);
...
const addWeek_Site_Plan = (req,res) => {
    var getquery = "SELECT name FROM sys.columns WHERE object_id = OBJECT_ID('Week_Site_Plan')";
    var Results = [];
    // console.log(query);
    sql.connect(config).then(() => {
        ...
        request.query(getquery, function (err, recordsets) {
            ...
            Results = recordsets.recordsets[0]
            let keysStatus = true
            var query = "INSERT INTO Week_Site_Plan Values( "
...
const updateWeek_Site_Plan = (req,res) => {
    var getquery = "SELECT name FROM sys.columns WHERE object_id = OBJECT_ID('Week_Site_Plan')";
    var Results = [];
    var cond = '';
    // console.log(query);
    sql.connect(config).then(() => {
        ...
        request.query(getquery, function (err, recordsets) {
            ...
            Results = recordsets.recordsets[0]
            let keysStatus = true
            var query = "UPDATE Week_Site_Plan SET "
            const keys = Object.keys(req.body)
            console.log(req.body)
            // var i = 0
            for (let i = 0; i < Results.length; i++) {
...
const deleteWeek_Site_Plan = (req,res) => {
    // var query = 'SELECT * FROM Week_Site_Plan';
    // console.log(query);
    ...
    sql.connect(config, function (err) {
        ...
        var request = new sql.Request();
        //Read Sql Statment From File
        request.query("DELETE FROM Week_Site_Plan " +
