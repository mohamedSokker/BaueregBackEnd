...
const getAllConstants = (req,res) => {
    ...
    if (limit) {
        query = 'SELECT TOP ' + limit + ' * FROM Constants'
    } else {
...
const addConstants = (req,res) => {
    var getquery = "SELECT name FROM sys.columns WHERE object_id = OBJECT_ID('Constants')";
    var Results = [];
    // console.log(query);
...
const updateConstants = (req,res) => {
    ...
    sql.connect(config).then(() => {
        ...
        request.query(getquery, function (err, recordsets) {
            ...
            Results = recordsets.recordsets[0]
            let keysStatus = true
            var query = "UPDATE Constants SET "
            const keys = Object.keys(req.body)
            console.log(req.body)
            // var i = 0
            for (let i = 0; i < Results.length; i++) {
...
const deleteConstants = (req,res) => {
    ...
    sql.connect(config, function (err) {
        ...
        var request = new sql.Request();
        //Read Sql Statment From File
        request.query("DELETE FROM Constants " +
