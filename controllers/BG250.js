...
const getAllBG250 = (req,res) => {
    var query = 'SELECT * FROM BG250';
    let { cond, limit, fullquery } = req.query;
    if (limit) {
        query = 'SELECT TOP ' + limit + ' * FROM BG250'
    } else {
        query = 'SELECT * FROM BG250'
    }
    if (cond) {
...
const addBG250 = (req,res) => {
    var getquery = "SELECT name FROM sys.columns WHERE object_id = OBJECT_ID('BG250')";
    var Results = [];
    // console.log(query);
    sql.connect(config).then(() => {
        ...
        request.query(getquery, function (err, recordsets) {
            ...
            Results = recordsets.recordsets[0]
            let keysStatus = true
            var query = "INSERT INTO BG250 Values( "
...
const deleteBG250 = (req,res) => {
    ...
    sql.connect(config, function (err) {
        ...
        var request = new sql.Request();
        //Read Sql Statment From File
        request.query("DELETE FROM BG250 " +
