...
const getAllBC250 = (req,res) => {
    var query = 'SELECT * FROM BC250';
    let { cond, limit, fullquery } = req.query;
    if (limit) {
        query = 'SELECT TOP ' + limit + ' * FROM BC250'
    } else {
        query = 'SELECT * FROM BC250'
    }
    if (cond) {
...
const addBC250 = (req,res) => {
    var getquery = "SELECT name FROM sys.columns WHERE object_id = OBJECT_ID('BC250')";
    var Results = [];
    // console.log(query);
    sql.connect(config).then(() => {
        ...
        request.query(getquery, function (err, recordsets) {
            ...
            Results = recordsets.recordsets[0]
            let keysStatus = true
            var query = "INSERT INTO BC250 Values( "
...
const deleteBC250 = (req,res) => {
    ...
    sql.connect(config, function (err) {
        ...
        var request = new sql.Request();
        //Read Sql Statment From File
        request.query("DELETE FROM BC250 " +
