...
const getAllOrders_TotalValue = (req,res) => {
    var query = 'SELECT * FROM Orders_TotalValue';
    let { cond, limit, fullquery } = req.query;
    if (limit) {
        query = 'SELECT TOP ' + limit + ' * FROM Orders_TotalValue'
    } else {
        query = 'SELECT * FROM Orders_TotalValue'
    }
    if (cond) {
...
const getOrders_TotalValue = (req,res) => {
    var query = 'SELECT * FROM Orders_TotalValue';
    // console.log(query);
    sql.connect(config, function (err) {
        if (err) console.log(err);
...
const addOrders_TotalValue = (req,res) => {
    var getquery = "SELECT name FROM sys.columns WHERE object_id = OBJECT_ID('Orders_TotalValue')";
    var Results = [];
    // console.log(query);
    sql.connect(config).then(() => {
        ...
        request.query(getquery, function (err, recordsets) {
            ...
            Results = recordsets.recordsets[0]
            let keysStatus = true
            var query = "INSERT INTO Orders_TotalValue Values( "
...
const updateOrders_TotalValue = (req,res) => {
    var getquery = "SELECT name FROM sys.columns WHERE object_id = OBJECT_ID('Orders_TotalValue')";
    var Results = [];
    var cond = '';
    // console.log(query);
    sql.connect(config).then(() => {
        ...
        request.query(getquery, function (err, recordsets) {
            ...
            Results = recordsets.recordsets[0]
            let keysStatus = true
            var query = "UPDATE Orders_TotalValue SET "
            const keys = Object.keys(req.body)
            console.log(req.body)
            // var i = 0
            for (let i = 0; i < Results.length; i++) {
...
const deleteOrders_TotalValue = (req,res) => {
    // var query = 'SELECT * FROM Orders_TotalValue';
    // console.log(query);
    ...
    sql.connect(config, function (err) {
        ...
        var request = new sql.Request();
        //Read Sql Statment From File
        request.query("DELETE FROM Orders_TotalValue " +
