...
const getAllSoil_Components = (req,res) => {
    var query = 'SELECT * FROM Soil_Components';
    let { cond, limit, fullquery } = req.query;
    if (limit) {
        query = 'SELECT TOP ' + limit + ' * FROM Soil_Components'
    } else {
        query = 'SELECT * FROM Soil_Components'
    }
    if (cond) {
...
const getSoil_Components = (req,res) => {
    var query = 'SELECT * FROM Soil_Components';
    // console.log(query);
    sql.connect(config, function (err) {
        if (err) console.log(err);
...
const addSoil_Components = (req,res) => {
    var getquery = "SELECT name FROM sys.columns WHERE object_id = OBJECT_ID('Soil_Components')";
    var Results = [];
    // console.log(query);
    sql.connect(config).then(() => {
        ...
        request.query(getquery, function (err, recordsets) {
            ...
            Results = recordsets.recordsets[0]
            let keysStatus = true
            var query = "INSERT INTO Soil_Components Values( "
...
const updateSoil_Components = (req,res) => {
    var getquery = "SELECT name FROM sys.columns WHERE object_id = OBJECT_ID('Soil_Components')";
    var Results = [];
    var cond = '';
    // console.log(query);
    sql.connect(config).then(() => {
        ...
        request.query(getquery, function (err, recordsets) {
            ...
            Results = recordsets.recordsets[0]
            let keysStatus = true
            var query = "UPDATE Soil_Components SET "
            const keys = Object.keys(req.body)
            console.log(req.body)
            // var i = 0
            for (let i = 0; i < Results.length; i++) {
...
const deleteSoil_Components = (req,res) => {
    // var query = 'SELECT * FROM Soil_Components';
    // console.log(query);
    ...
    sql.connect(config, function (err) {
        ...
        var request = new sql.Request();
        //Read Sql Statment From File
        request.query("DELETE FROM Soil_Components " +
