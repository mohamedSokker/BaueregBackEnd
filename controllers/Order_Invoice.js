...
const getAllOrder_Invoice = (req,res) => {
    var query = 'SELECT * FROM Order_Invoice';
    let { cond, limit, fullquery } = req.query;
    if (limit) {
        query = 'SELECT TOP ' + limit + ' * FROM Order_Invoice'
    } else {
        query = 'SELECT * FROM Order_Invoice'
    }
    if (cond) {
...
const getOrder_Invoice = (req,res) => {
    var query = 'SELECT * FROM Order_Invoice';
    // console.log(query);
    sql.connect(config, function (err) {
        if (err) console.log(err);
...
const addOrder_Invoice = (req,res) => {
    var getquery = "SELECT name FROM sys.columns WHERE object_id = OBJECT_ID('Order_Invoice')";
    var Results = [];
    // console.log(query);
    sql.connect(config).then(() => {
        ...
        request.query(getquery, function (err, recordsets) {
            ...
            Results = recordsets.recordsets[0]
            let keysStatus = true
            var query = "INSERT INTO Order_Invoice Values( "
...
const updateOrder_Invoice = (req,res) => {
    var getquery = "SELECT name FROM sys.columns WHERE object_id = OBJECT_ID('Order_Invoice')";
    var Results = [];
    var cond = '';
    // console.log(query);
    sql.connect(config).then(() => {
        ...
        request.query(getquery, function (err, recordsets) {
            ...
            Results = recordsets.recordsets[0]
            let keysStatus = true
            var query = "UPDATE Order_Invoice SET "
            const keys = Object.keys(req.body)
            console.log(req.body)
            // var i = 0
            for (let i = 0; i < Results.length; i++) {
...
const deleteOrder_Invoice = (req,res) => {
    // var query = 'SELECT * FROM Order_Invoice';
    // console.log(query);
    ...
    sql.connect(config, function (err) {
        ...
        var request = new sql.Request();
        //Read Sql Statment From File
        request.query("DELETE FROM Order_Invoice " +
