...
const getAllStocks_Items_Status = (req,res) => {
    var query = 'SELECT * FROM Stocks_Items_Status';
    let { cond, limit, fullquery } = req.query;
    if (limit) {
        query = 'SELECT TOP ' + limit + ' * FROM Stocks_Items_Status'
    } else {
        query = 'SELECT * FROM Stocks_Items_Status'
    }
    if (cond) {
...
const getStocks_Items_Status = (req,res) => {
    var query = 'SELECT * FROM Stocks_Items_Status';
    // console.log(query);
    sql.connect(config, function (err) {
        if (err) console.log(err);
...
const addStocks_Items_Status = (req,res) => {
    var getquery = "SELECT name FROM sys.columns WHERE object_id = OBJECT_ID('Stocks_Items_Status')";
    var Results = [];
    // console.log(query);
    sql.connect(config).then(() => {
        ...
        request.query(getquery, function (err, recordsets) {
            ...
            Results = recordsets.recordsets[0]
            let keysStatus = true
            var query = "INSERT INTO Stocks_Items_Status Values( "
...
const updateStocks_Items_Status = (req,res) => {
    var getquery = "SELECT name FROM sys.columns WHERE object_id = OBJECT_ID('Stocks_Items_Status')";
    var Results = [];
    var cond = '';
    // console.log(query);
    sql.connect(config).then(() => {
        ...
        request.query(getquery, function (err, recordsets) {
            ...
            Results = recordsets.recordsets[0]
            let keysStatus = true
            var query = "UPDATE Stocks_Items_Status SET "
            const keys = Object.keys(req.body)
            console.log(req.body)
            // var i = 0
            for (let i = 0; i < Results.length; i++) {
...
const deleteStocks_Items_Status = (req,res) => {
    // var query = 'SELECT * FROM Stocks_Items_Status';
    // console.log(query);
    ...
    sql.connect(config, function (err) {
        ...
        var request = new sql.Request();
        //Read Sql Statment From File
        request.query("DELETE FROM Stocks_Items_Status " +
