...
const getAllBauer_Equipments_Model = (req,res) => {
    var query = 'SELECT * FROM Bauer_Equipments_Model';
    let { cond, limit, fullquery } = req.query;
    if (limit) {
        query = 'SELECT TOP ' + limit + ' * FROM Bauer_Equipments_Model'
    } else {
        query = 'SELECT * FROM Bauer_Equipments_Model'
    }
    if (cond) {
...
const getBauer_Equipments_Model = (req,res) => {
    var query = 'SELECT * FROM Bauer_Equipments_Model';
    // console.log(query);
    sql.connect(config, function (err) {
        if (err) console.log(err);
...
const addBauer_Equipments_Model = (req,res) => {
    var getquery = "SELECT name FROM sys.columns WHERE object_id = OBJECT_ID('Bauer_Equipments_Model')";
    var Results = [];
    // console.log(query);
    sql.connect(config).then(() => {
        ...
        request.query(getquery, function (err, recordsets) {
            ...
            Results = recordsets.recordsets[0]
            let keysStatus = true
            var query = "INSERT INTO Bauer_Equipments_Model Values( "
...
const updateBauer_Equipments_Model = (req,res) => {
    var getquery = "SELECT name FROM sys.columns WHERE object_id = OBJECT_ID('Bauer_Equipments_Model')";
    var Results = [];
    var cond = '';
    // console.log(query);
    sql.connect(config).then(() => {
        ...
        request.query(getquery, function (err, recordsets) {
            ...
            Results = recordsets.recordsets[0]
            let keysStatus = true
            var query = "UPDATE Bauer_Equipments_Model SET "
            const keys = Object.keys(req.body)
            console.log(req.body)
            // var i = 0
            for (let i = 0; i < Results.length; i++) {
...
const deleteBauer_Equipments_Model = (req,res) => {
    // var query = 'SELECT * FROM Bauer_Equipments_Model';
    // console.log(query);
    ...
    sql.connect(config, function (err) {
        ...
        var request = new sql.Request();
        //Read Sql Statment From File
        request.query("DELETE FROM Bauer_Equipments_Model " +
