...
const getAllAppPlaceOrder = async (req, res) => {
  try {
    ...
    if (model["AppPlaceOrder"]) {
      ...
      for (let i = 0; i < model["AppPlaceOrder"].length; i++) {
        jsonStream.write(model["AppPlaceOrder"][i]);
      }

      // End the JSONStream serializer
      jsonStream.end();
    } else {
      getData("SELECT * FROM AppPlaceOrder").then((result) => {
    ...
    console.log(`AppPlaceOrder b ${memoryUsageBefore / (1024 * 1024)} MB`);
    console.log(`AppPlaceOrder a ${memoryDiff / (1024 * 1024)} MB`);
...
const getAppPlaceOrder = async (req, res) => {
  try {
    ...
    const result = await getOneData(cond, "AppPlaceOrder");
    // const cond = req.params.id;
...
const deleteAppPlaceOrder = async (req, res) => {
  try {
    const targetColVal = req.params.id;
    const result = await deleteData(targetColVal, "AppPlaceOrder");
    // const targetColVal = Object.values(req.params)[0];
