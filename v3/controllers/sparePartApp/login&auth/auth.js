    ...
    jwt.verify(token, process.env.TOKEN_SECRET_KEY, async (err, decode) => {
      ...
      try {
        ...
        if (model["AdminUsersApp"]) {
          console.log("auth from model");
          console.log(model["AdminUsersApp"][0]);
          Results = model["AdminUsersApp"].filter(
            (user) => user?.UserName === decode?.username
          );
        } else {
          console.log("auth from query");
          const query = `SELECT TOP 1  UserRole FROM AdminUsersApp WHERE UserName = '${decode?.username}'`;
