class Raw {
  ...
  decode(
    ...
    return new Promise(async (resolve, reject) => {
      try {
        await socket.waitBytes(
          rect.width * rect.height * (bitsPerPixel / 8),
          "Raw pixel data"
        );
        rect.data = socket.readNBytesOffset(
          rect.width * rect.height * (bitsPerPixel / 8)
        );
        ...
        for (let h = 0; h < rect.height; h++) {
          for (let w = 0; w < rect.width; w++) {
            ...
            if (bitsPerPixel === 8) {
              ...
              fb.writeUInt8(
                rect.data.readUInt8(bytePosOffset + red),
                fbBytePosOffset
              );
              fb.writeUInt8(
                rect.data.readUInt8(bytePosOffset + green),
                fbBytePosOffset + 1
              );
              fb.writeUInt8(
                rect.data.readUInt8(bytePosOffset + blue),
                fbBytePosOffset + 2
              );
