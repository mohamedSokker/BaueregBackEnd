        ...
        this.zlib.write(compressedData, async () => {
          ...
          while (tiles) {
            ...
            if (subEncoding === 129) {
              ...
              for (let h = 0; h < th; h++) {
                for (let w = 0; w < tw; w++) {
                  ...
                  if (bitsPerPixel === 8) {
                    ...
                    fb.writeIntBE(
                      this.unBuffer.readRgbPlusAlpha(red, green, blue),
                      fbBytePosOffset,
                      4
                    );
                    ...
                    fb.writeIntBE(
                      this.unBuffer.readRgba(red, green, blue),
                      fbBytePosOffset,
                      4
                    );
                  }
              ...
              const bitsPerIndex =
                subEncoding === 2 ? 1 : subEncoding < 5 ? 2 : 4;
              ...
              for (let x = 0; x < paletteSize; x++) {
                ...
                if (firstRle)
                  console.log("Cor da paleta: " + JSON.stringify(color));
                ...
                for (let w = 0; w < tw; w++) {
                  if (!runLength) {
                    ...
                    if (!(colorIndex & 128)) {
                      ...
                      while (runSize === 255) {
                        runLength += runSize;
                        await this.unBuffer.waitBytes(
                          1,
                          "paletterle runlength"
                        );
                        runSize = this.unBuffer.readUInt8();
                      }
                      runLength += runSize + 1;
            ...
            this._log(
              `Processing tile ${
                totalTiles - tiles
              }/${totalTiles} - SubEnc: ${subEncoding} - Size: ${tw}x${th} - BytesUsed: ${
                this.unBuffer.offset - initialOffset
              } - TotalRun: ${totalRun} - Runs: ${runs} - PaletteSize: ${
                palette.length
              }`,
              true,
              3
